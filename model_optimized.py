...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        batch_dim = 32 # the maximum batch dimension we'll support. This is because T4 has small mem.
        ...
        # hint: the first dimension is '2' because we need to store both the key and the value for each token in the sequence so TODO = (2, ?, ?, ?, ?)
    ...
    def decode(self, x): # note: x now only includes the tokns *after* the KV cache
        ...
        k = self.kv_cache[0,:B,:,:self.seq_pos+T,:]
        v = self.kv_cache[1,:B,:,:self.seq_pos+T,:]
...
class GPT(nn.Module):
    ...
    def generate_kv(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        for _ in range(max_new_tokens-1):
            ...
            logits = self.decode(new_tokens[:, -1:])
            # pluck the logits at the final position and scale by desired temperature
            logits = logits[:, -1, :] / temperature
            # optionally crop the logits to only the top k options
            if top_k is not None:
    ...
    def generate_speculative(self, idx, max_new_tokens, draft_model, temperature=1.0, top_k=None, num_speculative=4):
        ...
        while idx.size(1) < idx_length_original+max_new_tokens:
            ...
            idx_speculative = draft_model.generate(idx_cond, max_new_tokens=num_speculative, temperature=temperature, top_k=1) # TODO

            # obtain the logits from the main model by passing in the idx_speculative
            all_logits, _ = self.forward(idx_speculative)# TODO
            ...
            for i in range(idx_cond.size(1), idx_speculative.size(1)):
                ...
                if top_k is not None:
                  if top_k == 1:
                      idx_next = torch.argmax(logits, dim=-1).reshape((-1,1))
                      idx = torch.cat((idx, idx_next), dim=1) # using argmax here preserves RNG state
                      if not torch.equal(idx_next[:, 0], idx_speculative[:, i]):
                        break
                      else:
                        continue
                  v, _ = torch.topk(logits, min(top_k, logits.size(-1)))
